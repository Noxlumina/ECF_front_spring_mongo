        ...
        >
          ...
          <td>
            <MDBBtn color="warning">
              <router-link :to="{ name: 'Vehicule', params: { id: vehicule.id } }"
                >Modifier</router-link
              >
            </MDBBtn>
          </td>
          <td>
            <MDBBtn color="danger" @:click="removeItem(vehicule.id)">Supprimer</MDBBtn>
          </td>
        </tr>
      </tbody>
    </MDBTable>
    ...
    <MDBTable captionTop v-if="!isEditing" :class="$style.form">
      ...
      <tbody>
        <tr v-for="vehicule of vehicules" :key="vehicule.immatriculation">
          ...
          <td>
            <MDBBtn color="danger" v-on:click="removeItem(vehicule.id)"
              >Supprimer</MDBBtn
            >
          </td>
        </tr>
...
export default {
  ...
  async created() {
    try {
      const res = await http.get(`http://localhost:8090/vehicules`);
  ...
  methods: {
     ...
     */
    async addVehicule() {
      const res = await http.post(`http://localhost:8090/vehicules`, {
     ...
     */
    removeItem(id) {
      http.delete(`http://localhost:8090/vehicules/${id}`);
