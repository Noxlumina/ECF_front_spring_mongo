...
<template>
  ...
  <form :class="$style.form">
    ...
    <MDBTable captionTop v-if="isEditingLocataire" :class="$style.form">
      ...
      <tbody>
        <tr>
          <th scope="row">{{ locatairechoisi.id }}</th>
    ...
    <MDBBtn color="success" @:click="addLocation(vehiculechoisi.id)"
...
export default {
  ...
  async created() {
    try {
      ...
      const res1 = await http.get(`http://localhost:8090/vehicules`);
      this.vehicules = res1.data;
      const res2 = await http.get(`http://localhost:8090/locataires`);
  ...
  methods: {
    ...
    async getByImmatriculation(imma) {
      try {
        const res = await http.get(
          `http://localhost:8090/vehicules/immatriculation/${imma}`
        );
    ...
    async getByEmail(email) {
      try {
        const res = await http.get(`http://localhost:8090/locataires/email/${email}`);
    ...
    getPrice(start, end) {
      ...
      if (time > Date.now()) {
        ...
        this.totalprice = diffInDays * this.vehiculechoisi.cout;
    ...
    async update(voitureid) {
      try {
        await http
          .post(`http://localhost:8090/vehicules`, {
            immatriculation: this.vehiculechoisi.immatriculation,
            marque: this.vehiculechoisi.marque,
            modele: this.vehiculechoisi.modele,
            type: this.vehiculechoisi.type,
            id: voitureid,
            cout: this.vehiculechoisi.cout,
            disponibilite: false,
          })
