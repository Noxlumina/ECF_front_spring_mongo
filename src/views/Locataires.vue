        ...
        >
          ...
          <td>
            <MDBBtn color="warning">
              <router-link :to="{ name: 'Locataire', params: { id: locataire.id } }"
                >Modifier</router-link
              >
            </MDBBtn>
          </td>
          <td>
            <MDBBtn color="danger" v-on:click="removeItem(locataire.id)"
              >Supprimer</MDBBtn
            >
          </td>
        </tr>
...
export default {
  ...
  async created() {
    try {
      const res = await http.get(`http://localhost:8090/locataires`);
  ...
  methods: {
     ...
     */
    async addLocataire() {
      const res = await http.post(`http://localhost:8090/locataires`, {
        nom: this.nom,
        prenom: this.prenom,
        email: this.email,
     ...
     */
    removeItem(id) {
      http.delete(`http://localhost:8090/locataires/${id}`);
