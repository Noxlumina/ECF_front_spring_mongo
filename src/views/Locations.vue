...
<template>
  ...
  <MDBTable captionTop :class="$style.form">
    ...
    <tbody>
      <tr v-for="location of locations" :key="location.id">
        ...
        <td>
          <MDBBtn color="danger" v-on:click="removeItem(location.id)">Supprimer</MDBBtn>
        </td>
      </tr>
    </tbody>
  </MDBTable>
...
export default {
  ...
  async created() {
    try {
      const reslocation = await http.get(`http://localhost:8090/locations`);
  ...
  methods: {
     ...
     */
    removeItem(id) {
      http.delete(`http://localhost:8090/locations/${id}`);
